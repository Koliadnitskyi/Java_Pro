...
public class Implements implements Сounting, Forming, Searching {

    @Override
    public void countOccurance(List<String> array, String string) {
        int counter = 0;
        for (Object o : array) if (string.equals(o)) counter++;
    ...
    public void calcOccurance(List<String> array) {
        ...
        for (int u = 0; u < test.size(); u++) {
            for (int r = u; r < test.size() - 1; r++) if (test.get(u).equals(test.get(r))) test.remove(r);
        }
        for (String s : test) {
            int counter = 0;
            for (String value : array) if (s.equals(value)) counter++;
    ...
    public List<Integer> toList(int[] array) {
        ...
        for (int i : array) methodList.add(i);
        return methodList;
    }
    ...
    public List<Integer> findUnique(List<Integer> array) {
        for (int u = 0; u < array.size(); u++) {
            for (int r = u; r < array.size() - 1; r++) if (array.get(u).equals(array.get(r + 1))) array.remove(r + 1);
        }
        return array;
    }
}
